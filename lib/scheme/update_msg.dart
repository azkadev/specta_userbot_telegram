...
Future<Map?> apiUpdateMsg(Map message, {required UpdateTd update, required Tdlib tg, bool isLite = false}) async {
  ...
  if (message["reply_to_message_id"] != 0 && message["reply_in_chat_id"] != 0) {
    if (update.client_option["is_login_bot"] == false && msg["is_outgoing"] == true) {
      isLite = false;
    }
    if (update.client_option["is_login_bot"] == true && msg["is_outgoing"] == false) {
      isLite = false;
    }
  ...
  if (message["forward_info"] is Map) {
    Map forward_info = message["forward_info"];
    if (forward_info["date"] is int) {
      msg["forward_date"] = forward_info["date"];
    }
  }
      ...
      try {
        ...
        if (dataEntities["type"]["@type"] != null) {
          ...
          if (typeEntities == "text_mention" && dataEntities["type"]["user_id"] != null) {
            ...
            try {
              var fromResult = await tg.getChat(message["sender_id"]["user_id"], clientId: update.client_id);
              if (fromResult["ok"]) {
                fromJson = fromResult["result"];
              }
            } catch (e) {}
